...
export const ListSliderList = ({
  ...
  const handleOperation = (boolean, data) => {
    if (boolean) {
      index.current =
        index.current == serviceData.length - 1 ? 0 : (index.current += 1);
    } else {
      index.current =
        index.current == 0 ? serviceData.length - 1 : (index.current -= 1);
    }
  ...
  return (
    <ul className='flex flex-col gap-[40px] relative py-[40px] text-sliderNums w-1350:flex-row w-1350:py-[20px] w-530:gap-5'>
      <li
